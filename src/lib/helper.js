...
export async function createMemberInTransaction(
    ...
    // एजेंट डॉक्यूमेंट का संदर्भ (Reference for Agent Document)
    ...
    return await runTransaction(db, async (transaction) => {
        ...
        // **********************************************
        // 1. ALL READS (सभी दस्तावेज़ों को पहले पढ़ें)
        // **********************************************

        // 1a. प्रोग्राम दस्तावेज़ को पढ़ें (READ 1)
        const programDoc = await transaction.get(programDocRef);
        if (!programDoc.exists()) {
            throw new Error("Program Document does not exist!");
        }

        // 1b. एजेंट दस्तावेज़ को पढ़ें (READ 2) - यदि agentId मौजूद है
        let agentDoc = null;
        ...
        // 2a. नया सदस्य संख्या / पंजीकरण संख्या निर्धारित करें (Program Counter Logic)
        const currentMemberCount = programDoc.data().memberCount || 0;
        const newMemberCount = currentMemberCount + 1;
        // Note: Your provided logic for REG number:
        const newRegistrationNumber = 'R' + generate6DigitRegNo();
        // TIP: Transaction is best used with an incremental counter:
        // const newRegistrationNumber = 'REG' + newMemberCount.toString().padStart(5, '0');

        // 2b. सदस्य डेटा तैयार करें
        const newMemberData = {
            ...memberData,
        ...
        // **********************************************
        // 3. ALL WRITES (सभी अपडेट्स / Writes)
        // **********************************************

        // 3a. प्रोग्राम दस्तावेज़ में काउंटर अपडेट करें (WRITE 1)
        transaction.update(programDocRef, {
            memberCount: newMemberCount
        });

        // 3b. एजेंट दस्तावेज़ में प्रोग्राम-वार काउंटर अपडेट करें (WRITE 2) 
        if (agentId && agentDoc && agentDoc.exists()) {
            // memberData से programId प्राप्त करें
            const programId = memberData.programId;

            // वर्तमान प्रोग्राम-विशिष्ट गिनती प्राप्त करें
            ...
            // डॉट नोटेशन का उपयोग करके nested field को अपडेट करें
            transaction.update(agentDocRef, {
                [`programCounts.${programId}`]: newProgramCount
            });
            // यह सुनिश्चित करने के लिए कि programCounts मैप मौजूद है, इसे इस तरह भी लिखा जा सकता है:
            /*
            transaction.update(agentDocRef, {
                programCounts: {
                    ...programCounts, // पुराने काउंट्स को रखें
                    [programId]: newProgramCount // नया/अपडेटेड काउंट जोड़ें
                }
            });
            */
            // लेकिन dot notation [`programCounts.${programId}`] ज़्यादा सुरक्षित और स्केलेबल है।
        }

        // 3c. नया सदस्य सहेजें (WRITE 3)
