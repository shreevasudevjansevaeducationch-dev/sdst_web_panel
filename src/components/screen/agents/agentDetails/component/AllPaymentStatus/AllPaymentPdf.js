...
const AllPaymentPdf = ({ rowData = [], agentInfo = {} }) => {
  ...
  const renderPageHeader = () => (
    <>
      ...
      {
        TrsutData.topTitle.map((line, index) => (
          <Text key={index} style={styles.smallText}>{line}</Text>
        ))
      }
      </View>
      ...
      <View style={styles.headerSection}>
        ...
        <View style={styles.centerContent}>
          ...
          <View style={styles.schemeBox}>
            <Text style={styles.schemeText}>सभी योजना भुगतान सारांश</Text>
          </View>
        </View>

        <Image 
          src={TrsutData.logo}
          style={[styles.logoImage, {width: 90}]}
        />
      </View>
